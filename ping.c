...
{
	/*netdect -I eth0 -c 100 172.16.9.1*/
	argv[5] = "8.8.8.8";
	char buff[] = {0};
	struct hostent *hp;
	int ch, hold, packlen;
	...
	__system_property_get("dhcp.eth0.gateway", buff);
	...
	while ((ch = getopt(argc, argv, COMMON_OPTSTR "bRT:")) != EOF) {
		...
		case 'M':
			...
			break;
		case 'V':
			printf("ping utility, iputils-V20190815\n");
			exit(0);
		COMMON_OPTIONS
			common_options(ch);
			break;
	...
	loss = main_loop(icmp_sock, packet, packlen);
	if(loss > 0){
		printf("\n--------------------------------------------------\n");
		FILE *fd = NULL;
		fd = fopen("/sys/devices/ff290000.eth/stmmac-0\:04/phy_speed", "w");
		if(fd == NULL)
			return -1;
                fprintf(fd, "%d", 1);
